...
class MainActivity : AppCompatActivity() {
    ...
    private val aValueReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            ...
            Log. d(TAG, "Received A value from service: $aValue")
    ...
    private val gpsStatusReceiver = object : BroadcastReceiver() {
        override fun onReceive(context:  Context?, intent: Intent?) {
            val gpsReady = intent?.getBooleanExtra(MeasurementService. EXTRA_GPS_READY, false) ?: false
            Log.d(TAG, "GPS status received: ready=$gpsReady")
            isGpsReady = gpsReady
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val gpsStatusFilter = IntentFilter(MeasurementService.ACTION_GPS_STATUS)
        ContextCompat.registerReceiver(this, gpsStatusReceiver, gpsStatusFilter, ContextCompat.RECEIVER_NOT_EXPORTED)
    }
    ...
    private fun setupObservers() {
        viewModel.currentAValue.observe(this) { value ->
            Log. d(TAG, "ViewModel A value: $value, isServiceRunning:  $isServiceRunning")
            if (!isServiceRunning) {
    ...
    private fun validateNValue(): Int? {
        val text = binding.editTextN.text.toString()
        val value = text.toIntOrNull()
        return if (value != null && value >= 1 && value <= 99) value else null
    }

    private fun validateBValue(): Int? {
        val text = binding.editTextB.text.toString()
        val value = text.toIntOrNull()
        return if (value != null && value >= 1 && value <= 999) value else null
    }
